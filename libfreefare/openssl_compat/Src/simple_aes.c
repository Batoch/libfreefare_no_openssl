// Minimal AES-128 implementation (public domain, based on tiny-AES-c)
#include "simple_aes.h"

int simple_aes_set_encrypt_key(const uint8_t *userKey, int bits, SIMPLE_AES_KEY *key) {
    if (bits != 128) return -1;
    for (int i = 0; i < 16; ++i) key->rd_key[i] = userKey[i];
    key->rounds = 10;
    return 0;
}

int simple_aes_set_decrypt_key(const uint8_t *userKey, int bits, SIMPLE_AES_KEY *key) {
    return simple_aes_set_encrypt_key(userKey, bits, key);
}

// S-box
static const uint8_t sbox[256] = {
  
};

// Rcon
static const uint8_t Rcon[11] = {
  0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

static void KeyExpansion(const uint8_t *key, uint8_t *roundKeys) {
    memcpy(roundKeys, key, 16);
    uint8_t temp[4];
    int i = 4;
    int r = 1;
    while (i < 44) {
        memcpy(temp, roundKeys + 4 * (i - 1), 4);
        if (i % 4 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[r++];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
        }
        for (int j = 0; j < 4; ++j)
            roundKeys[4 * i + j] = roundKeys[4 * (i - 4) + j] ^ temp[j];
        ++i;
    }
}

static void SubBytes(uint8_t *state) {
    for (int i = 0; i < 16; ++i) state[i] = sbox[state[i]];
}

static void ShiftRows(uint8_t *state) {
    uint8_t tmp;
    tmp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = tmp;
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    tmp = state[3]; state[3] = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = tmp;
}

static uint8_t xtime(uint8_t x) { return (x << 1) ^ ((x >> 7) * 0x1b); }

static void MixColumns(uint8_t *state) {
    for (int i = 0; i < 4; ++i) {
        uint8_t *c = state + 4 * i;
        uint8_t t = c[0] ^ c[1] ^ c[2] ^ c[3];
        uint8_t tmp = c[0];
        c[0] ^= t ^ xtime(c[0] ^ c[1]);
        c[1] ^= t ^ xtime(c[1] ^ c[2]);
        c[2] ^= t ^ xtime(c[2] ^ c[3]);
        c[3] ^= t ^ xtime(c[3] ^ tmp);
    }
}

static void AddRoundKey(uint8_t *state, const uint8_t *roundKey) {
    for (int i = 0; i < 16; ++i) state[i] ^= roundKey[i];
}

void simple_aes_encrypt(const uint8_t *in, uint8_t *out, const SIMPLE_AES_KEY *key) {
    uint8_t state[16];
    uint8_t roundKeys[176];

    memcpy(state, in, 16);
    KeyExpansion(key->rd_key, roundKeys);
    AddRoundKey(state, roundKeys);
    for (int round = 1; round <= 9; ++round) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys + 16 * round);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys + 160);
    memcpy(out, state, 16);
}

// Inverse S-box
static const uint8_t rsbox[256] = {
  
};

static void InvSubBytes(uint8_t *state) {
    for (int i = 0; i < 16; ++i) state[i] = rsbox[state[i]];
}

static void InvShiftRows(uint8_t *state) {
    uint8_t tmp;
    tmp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = tmp;
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    tmp = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = tmp;
}

// multiplication functions for InvMixColumns
static uint8_t gf_mult(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) result ^= a;
        uint8_t carry = a & 0x80;
        a <<= 1;
        if (carry) a ^= 0x1b;
        b >>= 1;
    }
    return result;
}

// Precompute multiplication by constants used in InvMixColumns
static uint8_t gf_mult_0e(uint8_t x) { return gf_mult(0x0e, x); }
static uint8_t gf_mult_09(uint8_t x) { return gf_mult(0x09, x); }
static uint8_t gf_mult_0d(uint8_t x) { return gf_mult(0x0d, x); }
static uint8_t gf_mult_0b(uint8_t x) { return gf_mult(0x0b, x); }

static void InvMixColumns(uint8_t *state) {
    for (int i = 0; i < 4; ++i) {
        uint8_t *c = state + 4 * i;
        uint8_t s0 = c[0], s1 = c[1], s2 = c[2], s3 = c[3];
        
        c[0] = gf_mult_0e(s0) ^ gf_mult_0b(s1) ^ gf_mult_0d(s2) ^ gf_mult_09(s3);
        c[1] = gf_mult_09(s0) ^ gf_mult_0e(s1) ^ gf_mult_0b(s2) ^ gf_mult_0d(s3);
        c[2] = gf_mult_0d(s0) ^ gf_mult_09(s1) ^ gf_mult_0e(s2) ^ gf_mult_0b(s3);
        c[3] = gf_mult_0b(s0) ^ gf_mult_0d(s1) ^ gf_mult_09(s2) ^ gf_mult_0e(s3);
    }
}

void simple_aes_decrypt(const uint8_t *in, uint8_t *out, const SIMPLE_AES_KEY *key) {
    uint8_t state[16];
    uint8_t roundKeys[176];

    memcpy(state, in, 16);
    KeyExpansion(key->rd_key, roundKeys);
    
    // Initial AddRoundKey with the last round key (round 10)  
    AddRoundKey(state, roundKeys + 160);
    
    // Rounds 9 down to 1 (middle rounds)
    for (int round = 9; round >= 1; --round) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, roundKeys + 16 * round);
        InvMixColumns(state);
    }
    
    // Final round (round 0) - no InvMixColumns
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, roundKeys + 0);
    
    memcpy(out, state, 16);
}
